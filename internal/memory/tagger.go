...
Respond with S-Expression format (no markdown, no explanation):
(outcome "good" (confidence 0.8) (reason "brief explanation"))

Example:
(outcome "bad" (confidence 0.9) (reason "user corrected the AI twice"))`, content)
    ...
    payload := map[string]interface{}{
        ...
        "messages": []map[string]string{
            {
                "role":    "system",
                "content": "You are an objective conversation analyzer. Respond only with valid S-Expressions.",
            },
            {
                "role":    "user",
                "content": prompt,
    ...
    if t.llmClient != nil {
        ...
        if queueClient, ok := t.llmClient.(LLMCaller); ok {
            ...
            outcomeStr, confidence, reason, err := utils.ParseOutcome(rawContent)
            if err != nil {
                return nil, fmt.Errorf("failed to parse outcome S-Expression: %w (content: %s)", err, rawContent)
            }
    ...
    return nil, fmt.Errorf("queue client required for outcome analysis")
}
...
Return a flat S-Expression list of semantic tags (nouns, topics, technologies):
(concepts "python" "debugging" "api")
...
Respond with S-Expression only (no markdown, no explanation).`, content)
    ...
    payload := map[string]interface{}{
        ...
        "messages": []map[string]string{
            {
                "role":    "system",
                "content": "You are a concept extraction system. Respond only with valid S-Expressions.",
            },
            {
                "role":    "user",
                "content": prompt,
    ...
    if t.llmClient != nil {
        ...
        if queueClient, ok := t.llmClient.(LLMCaller); ok {
            ...
            // Use the new robust parser
            concepts, err := utils.ParseConcepts(rawContent)
            if err != nil {
                // No fallback. The parser in utils has strict correction. 
                // If it fails here, it's unrecoverable garbage.
                return nil, fmt.Errorf("failed to parse concept S-Expression: %w (content: %s)", err, rawContent)
            }
    ...
    // Queue client required - no direct HTTP bypass allowed
    return nil, fmt.Errorf("queue client required for concept extraction")
}
